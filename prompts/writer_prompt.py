...
caption_ko 는 반드시 아래 순서와 구조를 그대로 따라라:
  ...
  3. 💡 이용 꿀팁 — 아래 규칙을 **반드시** 따라라:
     ...
     🔑 EVERGREEN LOCAL SPOT: 주어진 로컬 핫스팟(장소)의 분위기와 매력을 극대화해서,
     읽는 사람이 **당장 방문하고 싶게** 만드는 한 문장을 골라라. 할인·품절·구매 같은
     단기성/쇼핑성 멘트는 절대 쓰지 마라 — 우리는 시간 지나도 유효한 에버그린 로컬
     콘텐츠만 만든다. 아래 6가지 중 콘텐츠 분위기에 가장 잘 맞는 **정확히 하나만**
     골라서 그대로 써라:
       ...
       e) "주말 나들이 코스로 강력 추천합니다."
       f) "오전 일찍 방문하면 한산하게 즐길 수 있어요."
...
caption_en: Instagram US / Reddit 용 영어 캡션. caption_ko 와 동일한 구조로 작성:
  ...
  3. 💡 Practical tip — same anti-hallucination rule as caption_ko:
     ...
     🔑 EVERGREEN LOCAL SPOT: Pick ONE line that maximizes the place's vibe and
     makes the reader want to visit RIGHT NOW. Never use sale/deal/sold-out/limited
     -stock language — this account only makes evergreen local content that stays
     valid over time. Pick exactly ONE of these six lines that best matches the spot:
       ...
       e) "Great spot for a weekend outing."
       f) "Going early in the morning means a quieter, calmer visit."
