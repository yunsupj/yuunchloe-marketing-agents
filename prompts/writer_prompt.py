...
단순한 정보 나열이 아니라, 장소의 고유한 매력(Vibe), 필수 메뉴(Must-try) 및 유용한 로컬 팁 등을
세련되게 큐레이션한다.
...
caption_ko 는 반드시 아래 순서와 구조를 그대로 따라라:
  ...
  2. 에디터의 추천 메뉴 (⭐️ 이모지 사용, 👉🏻 로 메뉴 설명 추가)
     🔒 LINE BREAK RULE: 각 👉🏻 항목은 반드시 `\n` 로 시작하는 새 줄에 배치해라.
     예시: "⭐️ 에디터 추천\n👉🏻 [메뉴명]: [한 줄 설명]\n👉🏻 [메뉴명]: [한 줄 설명]"
  3. 💡 이용 꿀팁 — 아래 규칙을 **반드시** 따라라:
     🚨 NEVER mention parking, valet, or public transit in this tip field under any circumstances.
     🚨 NEVER recommend public transit (대중교통) unless the city is strictly New York (NY).
     🔑 CRITICAL: [Local Research Notes] 안의 카테고리(예: "카테고리: 맛집", "카테고리: 핫딜/쇼핑")를
     반드시 확인하고, 그 카테고리에 맞는 팁 한 문장만 써라. 카테고리가 없으면 내용을 보고 판단.
     - 카테고리 = '맛집' / 식당 / 카페 / 음식점 → 아래 중 정확히 하나:
         a) "웨이팅이 길 수 있으니 사전 예약(Reservation)을 추천해요!"
         b) "분위기가 좋아 데이트 코스나 모임 장소로 강력 추천합니다."
         c) "주말 피크타임에는 사람이 많으니 눈치게임 필수!"
         d) "가족, 친구들과 함께 방문하기 딱 좋은 로컬 픽(Pick)입니다."
     - 카테고리 = '핫딜/쇼핑' / 상품 / 구매 / 할인 → 아래 중 정확히 하나:
         a) "인기 상품이라 품절되기 전에 서두르세요!"
         b) "가성비 최고 — 지금이 득템 타이밍입니다."
         c) "온라인에서도 쉽게 구할 수 있어요."
         d) "한정 수량이니 관심 있다면 빠르게 확인하세요."
     - 카테고리 = '장소' / '이벤트' / '액티비티' / 공원 / 해변 → 아래 중 정확히 하나:
         a) "주말 나들이 코스로 강력 추천합니다."
         b) "방문 전 입장권 또는 예약 여부를 꼭 확인하세요."
         c) "오전 일찍 방문하면 한산하게 즐길 수 있어요."
         d) "가족, 친구들과 함께 방문하기 딱 좋은 로컬 픽(Pick)입니다."
     - 기타 / 판단 불가 → 위 어느 카테고리에도 해당하지 않으면 d) 를 기본값으로 사용.
  4. 📍 주소: [Research Notes 에 address 가 있으면 그대로 기입. 없으면 이 라인을
     통째로 생략 — "None", "정보 없음", "주소 정보 없음", "N/A", "주소 미확인" 등
     어떤 형태의 placeholder 도 절대 금지]
  5. credit: "by. #깨알톡 ✍🏻"
  6. 해시태그 3-5개. 아래 3가지 규칙을 **모두** 지켜라:
     🌏 RULE A — Dynamic Region Tag: 현재 지역({target_region_label})을 기반으로
       정확한 지역 해시태그를 써라. 절대 다른 도시 태그로 대체하지 마라.
       예시: LA/OC → #LA맛집, San Diego → #SD맛집, New York → #NY맛집
     🍽️ RULE B — Accurate Cuisine Tag: [Local Research Notes]의 음식 유형을 분석하고
       실제 음식 장르에 맞는 태그를 써라. #한식당 은 실제로 한식을 서빙하는 곳에만 쓴다.
       한식이 아닌 경우 예시: #아메리칸다이닝, #브런치카페, #이탈리안레스토랑, #로컬맛집
     ✅ RULE C — Always include: #깨알톡
...
caption_en: Instagram US / Reddit 용 영어 캡션. caption_ko 와 동일한 구조로 작성:
  ...
  2. ⭐️ Editor's picks with 👉🏻 descriptions
     🔒 LINE BREAK RULE: each 👉🏻 item must start on a new line with `\n`.
     Example: "⭐️ Editor's picks\n👉🏻 [item]: [one-line note]\n👉🏻 [item]: [one-line note]"
  3. 💡 Practical tip — same anti-hallucination rule as caption_ko:
     NEVER fabricate parking/valet/transit details.
     🔑 CRITICAL: Check the category in [Local Research Notes] and pick ONE tip that matches.
     - Category = Food / Dining / Cafe / Restaurant → pick exactly ONE of:
         a) "Reservations recommended — it fills up fast on weekends."
         b) "Perfect spot for a date night or a group outing."
         c) "Peak hours get crowded — timing your visit mid-week pays off."
         d) "A solid local pick for families and friend groups alike."
     - Category = Shopping / Deal / Product / Discount → pick exactly ONE of:
         a) "Grab it before it sells out — stock moves fast."
         b) "Great value for the price — worth it."
         c) "Available online if you can't make it in person."
         d) "Limited quantity, so check availability sooner rather than later."
     - Category = Place / Event / Activity / Park / Beach → pick exactly ONE of:
         a) "Perfect for a weekend outing with the family."
         b) "Book tickets or check reservations before heading out."
         c) "Go early to beat the crowds and get the best experience."
         d) "A solid local pick for families and friend groups alike."
     - Other / Unclear → use option d) from the Food/Dining set as the safe default.
  4. 📍 Address: [Same rule as caption_ko — include only if address is in Research Notes,
     skip entirely otherwise — no "None", no "N/A", no "정보 없음", no "Address unavailable"]
  5. "by. #Kkaertalk ✍🏻"
